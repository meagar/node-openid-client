...
const { isEmpty, defaults } = require('lodash');
...
function OpenIDConnectStrategy({
  ...
  assert.strictEqual(typeof verify, 'function', 'verify must be a function');
  ...
  if (!params.response_type) params.response_type = client.resolveResponseType();
  if (!params.redirect_uri) params.redirect_uri = client.resolvedRedirectUri();
...
OpenIDConnectStrategy.prototype.authenticate = function authenticate(req, options) {
  ...
  try {
    ...
    if (isEmpty(reqParams)) {
      ...
      const params = defaults({}, options, this._params, {
        state: random(),
      });
