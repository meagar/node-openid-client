  ...
  describe('with keystore (as option)', function () {
    ...
    [{}, [], 'not a keystore', 2, true, false].forEach(function (notkeystore) {
      it(`validates it is a keystore (${typeof notkeystore} ${JSON.stringify(notkeystore)})`, function () {
        return issuer.Client.register({}, { keystore: notkeystore })
          .then(fail, ({ message }) => {
            expect(message).to.eql('keystore must be an instance of jose.JWKS.KeyStore');
          });
      });
    });
    ...
    it('does not accept oct keys', function () {
      ...
      return keystore.generate('oct', 32).then(() => {
        return issuer.Client.register({}, { keystore }).then(fail, ({ message }) => {
    ...
    it('does not accept public keys', function () {
      ...
      return issuer.Client.register({}, { keystore: new jose.JWKS.KeyStore(key) })
        .then(fail, ({ message }) => {
          expect(message).to.eql('keystore must only contain private EC or RSA keys');
        });
    });
